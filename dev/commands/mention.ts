...
import { Message as DMessage, User, Collection, Snowflake } from "discord.js";
...
@Discord()
export class MentionTruthCheck {
    ...
    async onMention(
        ...
        try {
            ...
            const aiApiKey = await MentionTruthCheck.secretManager.getSecret(process.env.AI_KEY);
            if (!aiApiKey) {
    ...
    private async findMessageToCheck(triggerMessage: DMessage): Promise<DMessage | null> {
        const channel = triggerMessage.channel;
        let lastMessageId: Snowflake | undefined = triggerMessage.id;
        ...
        while (messagesChecked < maxSearchDepth) {
            try {
                ...
                const fetchOptions: any = { limit: 20 };
                if (lastMessageId) {
